 /* Lab: Poll
 * Author: Emily Tumacder
 * Period: 2
 * Date: 11/14/19
A PollDisplayPanel holds the vote counts and
 displays the numbers and the pie chart for
the current vote counts. 
 */
// A PollDisplayPanel holds the vote counts and
// displays the numbers and the pie chart for
// the current vote counts.

import java.awt.Color;
import java.awt.Graphics;

import javax.swing.JButton;
import javax.swing.JPanel;

public class PollDisplayPanel extends JPanel
{
  private String name1, name2, name3, name4, name5;
  // STEP 1: Declare the int fields count1, count2, count3:
  
  int count1, count2, count3, count4, count5;
  
  final Color Red = new Color(255,100,110);
  final Color Purple = new Color(145,155,255);
  final Color Yellow = new Color(255,255,125);
  final Color Green = new Color(115,255,102);
  final Color Blue = new Color(75,204,255);

  // Constructor
  public PollDisplayPanel(String nm1, String nm2, String nm3, String nm4, String nm5)
  {
    setBackground(Color.WHITE);
    name1 = nm1;
    name2 = nm2;
    name3 = nm3;
    name4 = nm4;
    name5 = nm5;
    count1 = 0;   // optional
    count2 = 0;   // optional
    count3 = 0;   // optional
    count4 = 0;
    count5 = 0;
    

  }
 
  // Increments count1
  public void vote1()
  {
    // STEP 2: Finish vote1(), vote2(), and vote3()  
	  
	  if (count1 + count2 + count3 + count4 + count5 < 15)
	  {
	  count1++;
	  System.out.println(this);
	  }
	  else if (count1 + count2 + count3 + count4 + count5 >= 15)
	  {
	  resetVotes();
	  }
  }

  // Increments count2
  public void vote2()
  {
    // STEP 2: Finish vote1(), vote2(), and vote3()
	  if (count1 + count2 + count3 + count4 + count5 < 15)
	  {
	  count2++;
	  System.out.println(this);
	  }
	  else if (count1 + count2 + count3 + count4 + count5 >= 15)
	  {
	  resetVotes();
	  }
  }

  // Increments count3
  public void vote3()
  {
    // STEP 2: Finish vote1(), vote2(), and vote3()
	  if (count1 + count2 + count3 + count4 + count5 < 15)
	  {
	  count3++;
	  System.out.println(this);
	  }
	  else if (count1 + count2 + count3 + count4 + count5>= 15)
	  {
	  resetVotes();
	  }
  }

  public void vote4()
  {	
	  if (count1 + count2 + count3 + count4 + count5 < 15)
	  {
	  count4++;
	  System.out.println(this);
	  }
	  else if (count1 + count2 + count3 + count4 + count5 >= 15)
	  {
	  resetVotes();
	  }
  }
  
  public void vote5()
  {	
	  if (count1 + count2 + count3 + count4 + count5 < 15)
	  {
	  count5++;
	  System.out.println(this);
	  }
	  else if (count1 + count2 + count3 + count4 + count5 >= 15)
	  {
	  resetVotes();
	  }
  }
  
  // Returns a string representation of this object
  public String toString()
  {
  	// STEP 3: Complete the toString() method
  	// This method should return a String in the form:
  	// Tami: 1   Brian: 2   Liz: 0
	  
	   return
		   name1 + ": " + count1 + "	"
    	 + name2 + ": " + count2 + "	"
    	 + name3 + ": " + count3 + "	"
    	 + name4 + ": " + count4 + "	"
    	 + name5 + ": " + count5;
    
  }

  // Redefines JPanel's paintComponent to draw this pie chart
  public void paintComponent(Graphics g)
  {
    super.paintComponent(g);

    int w = getWidth();
    int h = getHeight();
    int x = w/2;
    int y = h/2;
    int r = Math.min(w, h) / 4;
    drawPieChart(g, x, y, r);
    drawLegend(g, x, y, r);
  }

  // Draws the pie chart.
  // To avoid gaps in the picture, the following algorithm is used:
  // 1. set fromDegree to 0;
  // 2. draw the red sector and increment fromDegree by its size
  // 3. draw the green sector and increment fromDegree by its size
  // 4. set the size of the blue sector to the remaining
  //    area, 360 - fromDegree, but not less than 0:
  //      degrees = Math.max(360 - fromDegree, 0);
  //    (Occasionally, due to rounding errors, fromDegree may become 361,
  //    for example, count1 = 5, count2 = 11, count3 = 0.)
  
  private void drawPieChart(Graphics g, int x, int y, int r)
  {
    int fromDegree = 0;
    int total = count1 + count2 + count3 + count4 + count5;
    

    if (total > 0 && total <= 15)
    {
      int degrees;
      
      g.setColor(Red);
      degrees = countToDegrees(count1, total);
      drawSector(g, x, y, r, fromDegree, degrees);
      fromDegree += degrees;

      // STEP 5: Finish the drawPieChart() method so that it correctly
      // draws all 3 sectors of the pie chart.
     
      g.setColor(Purple);
      degrees = countToDegrees(count2, total);
      drawSector(g, x, y, r, fromDegree, degrees);
      fromDegree += degrees;
      
      g.setColor(Yellow);
      degrees = countToDegrees(count3, total);
      drawSector(g, x, y, r, fromDegree, degrees);
      fromDegree += degrees;
      
      g.setColor(Green);
      degrees = countToDegrees(count4, total);
      drawSector(g, x, y, r, fromDegree, degrees);
      fromDegree += degrees;
      
      g.setColor(Blue);
      degrees = Math.max(360 - fromDegree, 0);
      drawSector(g, x, y, r, fromDegree, degrees);

    }
    else
    {
      g.setColor(Color.LIGHT_GRAY);
      drawSector(g, x, y, r, fromDegree, 360);
    }
    
  }
  

  // Draws the vote counts and the corresponding color squares.
  private void drawLegend(Graphics g, int x, int y, int r)
  {
    // Display the counts:
    y += (r + 20);
    g.setColor(Color.BLACK);

	// STEP 6: Finish the drawLegend() method by uncommenting and
	// completing the following three lines
    
    if (count1 + count2 + count3 + count4 + count5 < 15)
    {
    g.drawString( count1 + "" , x - r, y);

    g.drawString( count2 + "", x - r/2, y);

    g.drawString( count3 + "" , x, y);
    
    g.drawString( count4 + "", x + r/2, y);
	
    g.drawString( count5 + "", x + r, y);
    }
    
    else if (count1 + count2 + count3 + count4 + count5 >= 15)
    {
    	//Fill this out
    if (count1 == count2 && count1 > count3 && count1 > count4 && count1 > count5)
    g.drawString("Ana has tied with Bob with " + count1 + " votes", 90, 50);
    		    
    else if (count1 == count3 && count1 > count2 && count1 > count4 && count1 > count5)
    g.drawString("Ana has tied with Carl with " + count1 + " votes", 90, 50);
    		    
    else if (count1 == count4 && count1 > count2 && count1 > count3 && count1 > count5)
    g.drawString("Ana has tied with Desi with " + count1 + " votes", 90, 50);
    		    
    else if (count1 == count5 && count1 > count2 && count1 > count3 && count1 > count4)
    g.drawString("Ana has tied with Elle with " + count1 + " votes", 90, 50);
    
    else if (count2 == count3 && count2 > count1 && count2 > count4 && count2 > count5)
    g.drawString("Bob has tied with Carl with " + count2 + " votes", 90, 50); 
    
    else if (count2 == count4 && count2 > count1 && count2 > count3 && count2 > count5)
    g.drawString("Bob has tied with Desi with " + count2 + " votes", 90, 50); 
    
    else if (count2 == count5 && count2 > count1 && count2 > count3 && count2 > count4)
    g.drawString("Bob has tied with Elle with " + count2 + " votes", 90, 50);
    
    else if (count3 == count4 && count3 > count1 && count3 > count2 && count3 > count5)
    g.drawString("Carl has tied with Desi with " + count3 + " votes", 90, 50);
    
    else if (count3 == count5 && count3 > count1 && count3 > count2 && count3 > count4)
    g.drawString("Carl has tied with Elle with " + count3 + " votes", 90, 20);
    
    else if (count4 == count5 && count4 > count1 && count4 > count2 && count4 > count3)
    g.drawString("Desi has tied with Elle with " + count4 + " votes", 90, 50);
    
    else if (count1 == count2 && count2 == count3 && count3 == count4 && count4 == count5)
    g.drawString("Everyone has tied with " + count1 + " votes", 90, 50);
    
    //_____________________________________//
    
    else if (count1 > count2 && count1 > count3 && count1 > count4 && count1 > count5)
    g.drawString("Ana has won with " + count1 + " votes", 120, 50);
    
    else if (count2 > count1 && count2 > count3 && count2 > count4 && count2 > count5)
    g.drawString("Bob has won with " + count2 + " votes", 120, 50);
    
    else if (count3 > count1 && count3 > count2 && count3 > count4 && count3 > count5)
    g.drawString("Carl has won with " + count3 + " votes", 120, 50);
    
    else if (count4 > count1 && count4 > count2 && count4 > count3 && count4 > count5)
    g.drawString("Desi has won with " + count4 + " votes", 120, 50);
    
    else if (count5 > count1 && count5 > count2 && count5 > count3 && count5 > count4)
    g.drawString("Elle has won with " + count5 + " votes", 120, 50);
    
    }
	
	
    // Display the color squares:
    y += 5;
    x -= 2;
    g.setColor(Red);
    g.fillRect(x - r, y, 10, 10);
    g.setColor(Purple);
    g.fillRect(x - r/2, y, 10, 10);
    g.setColor(Yellow);
    g.fillRect(x, y, 10, 10);
    g.setColor(Green);
    g.fillRect(x + r/2, y, 10, 10);
    g.setColor(Blue);
    g.fillRect(x + r,  y, 10, 10);
  }

  // Returns the number of degrees in a pie slice that
  // corresponds to count / total, rounded to the nearest integer.
  private int countToDegrees(int count, int total)
  {
	// STEP 4: Complete the countToDegrees method so that it returns
	// the correct number of degrees
    return (int) (360.0*count/total +0.5);
  }


  // Draws a sector, centered at x, y, of radius r,
  // of angle measure degrees, starting at fromDegree.
  private void drawSector(Graphics g, int x, int y, int r, int fromDegree, int degrees)
  {
    if (degrees > 359)
      g.fillOval(x - r, y - r, 2 * r, 2 * r);
    else
      g.fillArc(x - r, y - r, 2 * r, 2 * r, fromDegree, degrees);
  }
  
  private void resetVotes()
  {
	  count1 = 0;
	  count2 = 0;
	  count3 = 0;
	  count4 = 0;
	  count5 = 0;
  }

  }

